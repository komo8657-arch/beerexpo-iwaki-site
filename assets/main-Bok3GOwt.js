(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function a(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function t(s){if(s.ep)return;s.ep=!0;const o=a(s);fetch(s.href,o)}})();document.addEventListener("DOMContentLoaded",()=>{f(),u(),m(),h(),g(),y(),p(),w(),document.getElementById("news-list")&&v(),document.getElementById("news-detail")&&L(),document.getElementById("news-all-list")&&k();const e=document.querySelector(".timetable-img-container");e&&e.addEventListener("click",()=>{const n=e.querySelector("img");if(n&&window.initModal){const a=document.querySelector(".modal"),t=a==null?void 0:a.querySelector("img");a&&t&&(t.src=n.src,a.classList.add("active"),document.body.style.overflow="hidden")}})});async function u(){try{const e=await fetch("/data/info.json");if(!e.ok)throw new Error("Info data not found");const n=await e.json();["event_name","event_name_en","catchphrase","date_short","date_html","time_html","venue_name","venue_address","fee_html","organizer"].forEach(t=>{document.querySelectorAll(`.info-${t}`).forEach(s=>{s.innerHTML=n[t]})})}catch(e){console.error("info.jsonの読み込みに失敗しました:",e)}}function f(){const e=document.querySelector(".nav-list");e&&(e.innerHTML=`
    ...
    `)}function p(){window.location.pathname==="/"||window.location.pathname.endsWith("/index.html")||window.location.pathname.endsWith("/"),document.querySelectorAll(".logo").forEach(e=>{e.innerHTML='<img src="/assets/img/logo3.png" alt="ビア博いわき" class="logo-img">'})}function w(){const e="https://www.facebook.com/share/1Dox9zMMon/",n="https://www.instagram.com/beer_expo_iwaki?igsh=a3d5M3U2endramh2",a=document.querySelector(".nav-list");if(a&&!a.querySelector(".nav-sns")){const s=document.createElement("li");s.className="nav-item nav-sns",s.innerHTML=`
        ...
        `)}function m(){const e=document.querySelector(".hamburger"),n=document.querySelector(".nav-list");!e||!n||(e.addEventListener("click",()=>{e.classList.toggle("active"),n.classList.toggle("active"),document.body.style.overflow=n.classList.contains("active")?"hidden":""}),n.querySelectorAll("a").forEach(a=>{a.addEventListener("click",()=>{a.parentElement.classList.contains("has-dropdown")||(e.classList.remove("active"),n.classList.remove("active"),document.body.style.overflow="")})}))}function h(){const e=document.querySelectorAll(".nav-item.has-dropdown");e.forEach(n=>{n.querySelector(".nav-link").addEventListener("click",t=>{window.innerWidth<=768&&(t.preventDefault(),n.classList.toggle("active"),e.forEach(s=>{s!==n&&s.classList.remove("active")}))})}),window.addEventListener("resize",()=>{window.innerWidth>768&&e.forEach(n=>n.classList.remove("active"))})}function g(){document.querySelectorAll(".accordion-header").forEach(n=>{n.addEventListener("click",()=>{const a=n.parentElement,t=a.classList.contains("active");a.classList.toggle("active",!t)})})}function y(){const e=document.querySelector(".modal"),n=e==null?void 0:e.querySelector("img"),a=e==null?void 0:e.querySelector(".modal-close");if(!e)return;document.querySelectorAll("[data-modal-src], .gallery-item img, .map-image").forEach(s=>{s.addEventListener("click",()=>{const o=s.dataset.modalSrc||s.src;n&&(n.src=o),e.classList.add("active"),document.body.style.overflow="hidden"})}),a==null||a.addEventListener("click",()=>t()),e.addEventListener("click",s=>{s.target===e&&t()}),document.addEventListener("keydown",s=>{s.key==="Escape"&&t()});function t(){e.classList.remove("active"),document.body.style.overflow=""}}async function v(){const e=document.querySelector("#news-list");if(e)try{const n=await fetch("/data/news.json");if(!n.ok)throw new Error("News data not found");const t=(await n.json()).sort((r,c)=>r.pinned&&!c.pinned?-1:!r.pinned&&c.pinned?1:new Date(c.date)-new Date(r.date)),o=window.innerWidth<=768?5:8,i=t.slice(0,o);e.innerHTML=i.map(r=>`
